...
//	private final long mMinRefreshingTime = 2 * 1000;  //最小显示时间
	private long mStartRefreshingTime ;
	// ===========================================================
	// Constructors
	// ===========================================================
	...
	/***
	 * dynamical show header refreshing ,add by wudeng
	 */
	...
	/**
	 * dynamical show footer refreshing ,add by wudeng
	 */
	...
	/***
	 * dynamical to stop pull_refresh_from_end ,add by wudeng
	 */
	protected void stopLoadingFooterMode() {
		if (mMode == Mode.BOTH) {
	...
	/***
	 * dynamical to stop pull_refresh_from_start ,add by wudeng
	 */
	protected void stopLoadingHeaderMode() {
		if (mMode == Mode.BOTH) {
	 ...
	 * 重置原来的mode
	 */
	public void resetOriginMode() {
		post(new Runnable() {
			@Override
			public void run() {
				setMode(mOriginalMode);
			}
		});
	}

	/**
	 * set night mode changed,added by wudeng
	 */
    public void setNightModeChanged() {
        setNightModeChanged(true);
    }

	public void setNightModeChanged(boolean enableNightMode) {
		if (enableNightMode) {
		    setBackgroundColor(StyleUtils.getAppBackgroundColor(getContext()));
            getRefreshableView().setBackgroundColor(StyleUtils.getAppBackgroundColor(getContext()));
            getLoadingLayoutProxy().setNightModeChanged();
        } else {
	...
	public void setNightMode(boolean isNight) {
		if (isNight) {
            ...
            getLoadingLayoutProxy().setNightMode(isNight);
        } else {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (isReadyForPull()||isReadyForZoomStart()) {
				...
				if (absDiff > mTouchSlop
						&& (!mFilterTouchEvents || absDiff > Math
								.abs(oppositeDiff))) {
					//当currentMode为PULL_FROM_END时满足zoom条件
					 if (diff >= 1f && isReadyForZoomStart()) {
                       mLastMotionY = y;
                       mLastMotionX = x;
	...
	/**
	 * 延迟onRefreshComplete，避免一些loading无法消失的bug
	 *
	 * @param delayMillis
	 */
	public final void onPostRefreshComplete(long delayMillis) {
	...
	public final void onRefreshComplete() {
		if (isRefreshing()) {
//			if(mStartRefreshingTime != 0){
//				long curTime = System.currentTimeMillis();
//				if(curTime - mStartRefreshingTime < mMinRefreshingTime){
//					long difTime = mStartRefreshingTime + mMinRefreshingTime - curTime;
//
//					postDelayed(new Runnable() {
//						@Override
//						public void run() {
...
//			}
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP: {
			if (mIsBeingDragged) {
				mIsBeingDragged = false;
				if (isReadyForZoomStart()) {
					if (isZooming) {
						onZoomFinished(mCurrentMode);
						if (onPullZoomListener != null) {
							onPullZoomListener.onZoomEnd();
						}
						isZooming = false;
					}
				}
	 ...
	 * @return
	 */
	protected boolean isReadyForZoomStart(){return false;}
	/**
	...
	protected void onRefreshing(final boolean doScroll) {
		...
		mStartRefreshingTime = System.currentTimeMillis();
		if (doScroll) {
	...
	boolean releaseVisible = true;
	public void setReleaseLabelVisible(boolean visible){
		releaseVisible = visible;
	}
	...
	public void OnRefreshSuccess() {
		...
		// @Override
		// public void run() {
		// Log.v("wu", "OnRefreshSuccess()");
		// switch (mCurrentMode) {
		// case PULL_FROM_END:
		// mFooterLayout.refreshSuccess();
		// break;
		// case PULL_FROM_START:
		// Log.v("wu", "mHeaderLayout.refreshSuccess()");
		// mHeaderLayout.refreshSuccess();
		// break;
		// default:
		// break;
		// }
		// // onPostRefreshComplete(500);
		// }
		// }, 300);
		onPostRefreshComplete(500);
	}
	...
	public void OnNetworkError() {
		...
		// switch (mCurrentMode) {
		// case PULL_FROM_END:
		// mFooterLayout.networkError();
		// break;
		// case PULL_FROM_START:
		// mHeaderLayout.networkError();
		// break;
		// default:
		// break;
		// }
	...
	protected final void onRestoreInstanceState(Parcelable state) {
		if (state instanceof Bundle) {
			...
			mOriginalMode = Mode.mapIntToValue(bundle.getInt(STATE_MODE, 0));
			setMode(mOriginalMode);
		...
		postDelayed(new Runnable() {
            ...
            public void run() {
                if (!ContextUtils.checkNetworkConnection(getContext())) {
                    OnNetworkError();
                    ContextUtils.showToast(getContext(),
                            R.string.noconnectionremind);

                    if (mOnNetworkErrorListener != null) {
                        mOnNetworkErrorListener.onNetworkError();
                    }
                    if(isCheckNetWorkNextPage){
                        return;
                    }
                }

                if (null != mOnRefreshListener) {
                    mOnRefreshListener.onPullDownToRefresh(PullToRefreshBase.this);
                } else if (null != mOnRefreshListener2) {
                    if (mCurrentMode == Mode.PULL_FROM_START) {
                        resetOriginMode(); // 下拉刷新自动重置mMode
                        mOnRefreshListener2
                                .onPullDownToRefresh(PullToRefreshBase.this);
                    } else if (mCurrentMode == Mode.PULL_FROM_END) {
                        mOnRefreshListener2
                                .onPullUpToRefresh(PullToRefreshBase.this);
                    }
                }
	...
	private void init(Context context, AttributeSet attrs) {
		if (DEBUG) {
			Log.v(LOG_TAG, "init(Context context, AttributeSet attrs)");
		}
		switch (getPullToRefreshScrollDirection()) {
		case HORIZONTAL:
		...
		if (mRefreshableView instanceof ListView) {
			((ListView) mRefreshableView).setHeaderDividersEnabled(false);
	...
	private void pullEvent() {
		...
		if (isReadyForZoomStart()) {
			setHeaderZoom(newScrollValue);
			isZooming = true;
			if (onPullZoomListener != null) {
				onPullZoomListener.onZooming(newScrollValue);
			}
		}
		if(newScrollValue==0){
			isZooming=false;
		}
		if (!(isReadyForZoomStart() && mCurrentMode == Mode.PULL_FROM_END)) {
    ...
    public void setCheckNetWorkNextPage(boolean isCheckNetWorkNextPage) {
        this.isCheckNetWorkNextPage = isCheckNetWorkNextPage;
    }
